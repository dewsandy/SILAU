...
require('config/config.php');
	...
	// Page header
		...
		function Footer()
		{
			...
			// Page number
			$this->Cell(0,10,'Copyright : Computer Engineering A 2015',0,0,'C');
	...
	$pdf->Cell(4);
	$pdf->Cell(19.5,0,'Transaksi : '.$dayCode.' , '.$d.'',0,'L'); 
	$pdf->Ln(10); 
	$pdf->Cell(4);
	$pdf->Cell(35,7,'Nomer',1,0,'C');
	$pdf->Cell(35,7,'Makanan',1,0,'C');
	$pdf->Cell(35,7,'Minuman',1,0,'C'); 
	$pdf->Cell(35,7,'Total',1,0,'C'); 
	$pdf->Cell(35,7,'Tanggal Bayar',1,0,'C'); 
	$pdf->Ln(); 
	
	$pdf->SetFont('Times','',12);
	$hasil= $conn -> query("SELECT * FROM transaksi WHERE beli LIKE '".$tod."' ORDER BY nomer ASC");
	while($row=$hasil -> fetch_assoc()){ 
		$pdf->Cell(4);
		$pdf->SetFillColor(255,255,255); 
		$pdf->Cell(35,7,$row['nomer'],1,0,'L',true); 
		$pdf->Cell(35,7,'Rp					 			 		 '.number_format($row['makanan']).'',1,0,'L',true); 
		$pdf->Cell(35,7,'Rp										 '.number_format($row['minuman']).'',1,0,'L',true); 
		$pdf->Cell(35,7,'Rp 									 '.number_format($row['total']).'',1,0,'L',true);
		$pdf->Cell(35,7,$d,1,0,'L',true);
	...
	$pdf->SetFont('Times','B',12);
	$pdf->Cell(4);
	$pdf->Cell(35,5,'Kantin PENS',0,0,'L',true);
	$pdf->Ln(10);
	$pdf->Cell(4);
	$pdf->Cell(35,5,'ttd',0,0,'L',true);
	$pdf->Ln(10);
	$pdf->Cell(4);
	$pdf->Cell(35,5,''.$_SESSION['nama'].'',0,0,'L',true);
	$pdf->Output();
?>
