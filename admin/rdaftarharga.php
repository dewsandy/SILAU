...
require('config/config.php');
	...
	$pdf->Cell(80,7,'Jenis Laundry',1,0,'C');
	$pdf->Cell(80,7,'Harga',1,0,'C'); 
	
	$pdf->Ln(); 
	
	$pdf->SetFont('Times','',12);
	$nomor=1;
	$hasil= mysqli_query($conn,"SELECT * FROM harga h JOIN jenis_laundry j ON j.ID_LAUNDRY=h.JENIS_LAUNDRY");
	while($row=$hasil -> fetch_assoc()){ 
		$pdf->Cell(4);
		$pdf->SetFillColor(255,255,255); 
		$pdf->Cell(20,7,$nomor++,1,0,'C',true); 
		$pdf->Cell(80,7,$row['NAMA_LAUNDRY'],1,0,'C',true); 
		$pdf->Cell(80,7,'Rp					 			 		 '.number_format($row['HARGA']).'',1,0,'C',true);  
		$pdf->Ln(); 
	} 
	/*
